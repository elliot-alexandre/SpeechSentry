...
function loadModel(modelDirectory) {
  ...
  fs.access(modelDirectory, (err) => {
    if (err) throw `${err}: file ${modelDirectory} not found.`;
  });
...
function createRecognizer(
  ...
  if (alternatives) recognizer.setMaxAlternatives(alternatives);
 ...
 *
 */
async function transcriptFromFile(
  fileName,
  model,
  {
    multiThreads = true,
  ...
  return new Promise((resolve, reject) => {
    ...
    fs.access(fileName, (err) => {
      if (err) return reject(`${err}: file ${fileName} not found.`);
    });

    if (DEBUG) setTimer("createRecognizer");
    ...
    wfReader.on("format", async ({ audioFormat, sampleRate, channels }) => {
      if (audioFormat != 1 || channels != 1)
        return reject(
          `${fileName}: audio file (sample rate: ${sampleRate}) must be WAV format mono PCM.`
        );
...
function transcriptEventsFromFile(
  ...
  // validate audiofile existence, async
  fs.access(fileName, (err) => {
    if (err) throw `${err}: file ${fileName} not found.`;
  });
  ...
  wfReader.on("format", async ({ audioFormat, sampleRate, channels }) => {
    ...
    for await (const data of pcmChunks) {
      ...
      // By Nicolay Shmirev. See: https://github.com/alphacep/vosk-api/issues/590#issuecomment-863065813
      //
      if (end_of_speech) event.emit(END_OF_SPEECH_EVENT, recognizer.result());
      else {
 ...
 *
 */
async function transcriptFromBuffer(
  buffer,
  model,
  {
    multiThreads = true,
  ...
  // const end_of_speech = recognizer.acceptWaveform(data)
  //
  if (multiThreads) await recognizer.acceptWaveformAsync(buffer);
  else recognizer.acceptWaveform(buffer);
 ...
 *
 */
async function transcriptEventsFromBuffer(
  buffer,
  model,
  {
    multiThreads = true,
...
function checkArgs(args) {
  ...
  if (!modelDirectory) helpAndExit();

  if (!audioFile) helpAndExit();
  ...
  return {
    ...
    grammar: grammar
      ? grammar.split(",").map((sentence) => sentence.trim())
      : undefined,
...
function statistics(
  ...
  console.log(
    `grammar              : ${grammar ? grammar : "not specified. Default: NO"}`
  );
  console.log(
    `sample rate          : ${sampleRate ? sampleRate : "not specified. Default: 16000"}`
  );
...
async function main() {
  ...
  try {
    transcriptEvents = transcriptEventsFromFile(audioFile, model, {
      grammar,
      sampleRate,
      alternatives,
      words,
  ...
  transcriptEvents.on(FINAL_RESULT_EVENT, (data) => {
    ...
    if (textOnly) {
      ...
      if (!textOnly && tableevents) printResultsAsTable(results);
